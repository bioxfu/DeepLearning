...
def tain():
    ...
    for pre_train_model in pre_train_models:
        ...
        sp = preprocessor.SimplePreprocessor(image_size[pre_train_model], image_size[pre_train_model])
        pp = preprocessor.PatchPreprocessor(image_size[pre_train_model], image_size[pre_train_model])
        mp = preprocessor.MeanPreprocessor(means['R'], means['G'], means['B'])
        itap = preprocessor.ImageToArrayPreprocessor()
